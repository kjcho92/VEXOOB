...
#pragma config(Sensor, I2C_1,  BackRight_I2C,  sensorQuadEncoderOnI2CPort,    , AutoAssign )
#pragma config(Sensor, I2C_2,  Launcher_I2C,   sensorQuadEncoderOnI2CPort,    , AutoAssign )
...
#pragma config(Motor,  port6,           Launcher3,     tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port7,           Launcher4,     tmotorVex393_MC29, openLoop, reversed, encoderPort, I2C_2)
#pragma config(Motor,  port8,           BackLeft,      tmotorVex393_MC29, openLoop, driveLeft)
#pragma config(Motor,  port9,           BackRight,     tmotorVex393_MC29, openLoop, driveRight, encoderPort, I2C_1)
...
void LauncherDown_Helper()
{
	...
	long startTime = nPgmTime;
	while(nPgmTime - startTime < 100)
	{}

	while(SensorValue[LauncherReady] == 0 && nPgmTime - startTime < 350)
	{
	}
...
void TravelToTheOtherSide()
{
	...
	// Move forward
	nMotorEncoder(BackRight) = 0;
	EncoderForBack(50, 1800);

	wait1Msec(1000);

	// Turn left to the other side
	nMotorEncoder(BackRight) = 0;
	EncoderRotate(50, 375);

	wait1Msec(1000);

	// Move backward
	nMotorEncoder(BackRight) = 0;
	EncoderForBack(-60, 1500);
	...
	while (true)
	{
		...
		else if (btn8r == 1)
		{
			...
			else
			{
				...
				if (SensorValue[BallDispenserPosition] <= 550)
				{
					// Open
					...
					clearTimer(T2);
					//wait1Msec(1000);
					while(SensorValue[BallDispenserPosition] < 700 && time1[T2] < 5000)
					{
					}
				...
				{	// Close
					...
					clearTimer(T2);
					while(SensorValue[BallDispenserPosition] > 400 && time1[T2] < 5000)
					{
					}
...
void AutoLaunchBall_Full_Helper()
{
	...
	if (SensorValue[BallLoaded] == 1 || (baseLineSensor - SensorValue[LineBallLoaded]) > lineSensorOffset || ballLoaded == 1)
	{
		...
		if (baseLineSensor - SensorValue[LineBallLoaded] <= lineSensorOffset)
		{
			startTask(LauncherStop);
			return;
		}
