...
task LauncherUp_Mid()
{
	...
	int local_positionToStop = SensorValue[LauncherPosition] + 70;
	...
	//while(SensorValue[LauncherPosition] < local_positionToStop && time1[T1] < 500)
	//while(time1[T1] < 70)
	unsigned long time = nPgmTime + 75;
	while(nPgmTime < time)
	{
	}
		...
		else if (btn8r == 1)
		{
			...
			else
			{
				...
				if (SensorValue[BallDispenserPosition] >= 1300)
				{
					...
					clearTimer(T2);
					while(SensorValue[BallDispenserPosition] > 1080 && time1[T2] < 5000)
					{
					}
				...
				{	// Close
					...
					clearTimer(T2);
					while(SensorValue[BallDispenserPosition] < 1430 && time1[T2] < 5000)
					{
					}
